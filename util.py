...
#         print('{}{}: {}'.format(move[1], move[0],moveIntegerized))
...
def makeInputOutput(moveList, scraped=True):
    '''
    moveList - string representing all move in a game
    scraped - True - this movelist was scraped False - a movelist from a hex game
    winner - string - white or black
...
#         inputBoard.append(getBoard(tempBoard))
        ...
        #Out of 13*13= 169 possible board squares
        moveIntegerized = 13*(int(move[1])-1) + move[0]
        moveVector = np.zeros(13*13)
        moveVector[moveIntegerized-1] = 1
        # print('{}{}: {}'.format(move[1], move[0],moveIntegerized))
        # Store which square the piece went
        outputMove.append(moveVector)
        # Append piece to the board 1 is black 2 is white
        tempBoard.addPiece(move, 1 if black else 2)
        black = not(black)
#     print(tempBoard.chains)
#     print(tempBoard.__str__())
    # print(inputBoard[1:])
    # print(outputMove[1:])
    return inputBoard[1:], outputMove[1:]
...
def makeInputOutputTotal(moveLists, scraped=True):
    
    total = map(lambda args: makeInputOutput(args[0], args[1]),
                zip(moveLists, [scraped]*len(moveLists)))
    board_states_cumulative, move_action_cumulative = zip(*total)
    return (np.concatenate(board_states_cumulative, axis=0),
            np.concatenate(move_action_cumulative, axis=0))
